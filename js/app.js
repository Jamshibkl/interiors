...
if(cg){
  CAREERS.forEach((c,i)=>{
    ...
    const body=`Hi Kaveri Interiors,%0A%0AI am interested in applying for the ${c.role} role.%0A%0AName:%0AExperience:%0APortfolio:%0A%0AThank you.`;
    const el=document.createElement('div');
    ...
    el.innerHTML=`<b>${c.role}</b><span class="career-type">${c.type}</span><p>${c.desc}</p>
      <a class="career-apply" href="mailto:interiorsbykaveri@gmail.com?subject=${encodeURIComponent('Application: '+c.role)}&body=${body}">Apply →</a>`;
    cg.appendChild(el);
  });
}
